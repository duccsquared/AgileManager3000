...
initFromDB().then(() => loadDataFromDB())
...
let sprintIndex = localStorage.getItem("sprintToEdit")
...
// function to load back the data of the sprint from Database
function loadDataFromDB(){

    // HTML referrencing
    sprintNameRef = document.getElementById("newSprintName")
    startDateRef = document.getElementById("startDate")
    endDateRef = document.getElementById("endDate")

    sprintName = Sprint.sprintList[sprintIndex].name
    sprintStartDate = Sprint.sprintList[sprintIndex].startDate
    sprintEndDate = Sprint.sprintList[sprintIndex].endDate

    // Set the input box to the data of the sprint 
    sprintNameRef.value = sprintName
    startDateRef.value = sprintStartDate
    endDateRef.value = sprintEndDate

    // Reload all the tasks currently stored in the sprint 
    for ( let index in Sprint.sprintList[sprintIndex].taskIDList){
        for ( let j in Task.taskList){
            if (Sprint.sprintList[sprintIndex].taskIDList[index] === Task.taskList[j].id ){
                temporaryTasksInSprint.push(Task.taskList[j])
            }
        }
...
function saveEdit(){
    ...
    for (let index in Sprint.sprintList) {
        ...
        if (Sprint.sprintList[index].name !== Sprint.sprintList[sprintIndex].name){
            ...
            if (isPermitted == false){
                ...
                if (isAccepted == true) {
                    ...
                    if (isValid == false) {
                        ...
                        Sprint.sprintList[sprintIndex].name = sprintName
                        Sprint.sprintList[sprintIndex].startDate = sprintStartDate
                        Sprint.sprintList[sprintIndex].endDate = sprintEndDate

                        Sprint.sprintList[sprintIndex].taskIDList = []

                        for (let index in temporaryTasksInSprint) {

                            
                            Sprint.sprintList[sprintIndex].taskIDList.push(temporaryTasksInSprint[index].id)
                        }
                        Sprint.toDB()
